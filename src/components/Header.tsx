...
export default function Header() {
  ...
  return (
    <header className="absolute top-0 left-0 w-full z-20 bg-white/70 text-black">
      ...
      {/* Animated Dropdown */}
     <div
  className={`md:hidden origin-top-right fixed top-18 right-0 w-1/2 transition-all duration-300 ease-in-out
    ${isOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0 pointer-events-none'}
    bg-white/70 px-4 py-4 space-y-2 transform shadow-lg rounded-bl-lg
  `}
